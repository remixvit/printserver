...
| `profile_code` | string | Код профиля — **кириллица поддерживается** |
| `profile_name` | string | Название профиля — **кириллица поддерживается** |
| `order_title` | string | Название заказа — **кириллица поддерживается** |
...
> ✅ **Кириллица поддерживается** во всех строковых полях. JSON должен передаваться в UTF-8, сервер автоматически перекодирует в CP1251 для принтера.
